            ...
            <tbody>
              {mock.map((member, index) => (
                <tr key={member.email} className={`border-t border-white/5 ${index % 2 === 0 ? 'bg-slate-950/70' : 'bg-slate-900/80'}`}>
                  ...
                  <td className="px-4 py-4"><span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${member.active ? 'bg-emerald-500/15 text-emerald-200' : 'bg-slate-500/15 text-slate-300'}`}>{member.active ? 'Active' : 'Pending invite'}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
