...
export default function RegisterForm(){
  const steps = ['Business','Owner','Legal','Hours','Logo']
    ...
    <div className="space-y-6">
      <Card className="bg-slate-950/90">
        ...
        <div className="space-y-6">
          ...
          {active===3 && (
            <div className="grid gap-4 md:grid-cols-2"><Input label="FSSAI number" placeholder="112233445566" /><Input label="PAN number" placeholder="AABCU1234D" /></div>
          )}
          {active===4 && (
            <div className="space-y-5">
              <Input label="Address" placeholder="123 Main Street, Mumbai" />
              <label className="flex w-full cursor-pointer items-center justify-between rounded-3xl border border-white/10 bg-white/5 px-4 py-4 text-sm text-slate-200 transition hover:border-violet-300">
