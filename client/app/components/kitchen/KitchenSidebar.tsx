...
export default function KitchenSidebar({ isOpen, onClose }: KitchenSidebarProps) {
 ...
 const menuItems = [
 { name: "Active Orders", icon: LayoutGrid, path: "/kitchen" },
 { name: "Account", icon: User, path: "/kitchen/account" },
 ];
 ...
 {/* Sidebar */}
 <aside className={`fixed top-0 left-0 h-screen w-28 flex flex-col items-center py-6 glass-light border-r border-white/50 z-50 transform transition-transform duration-150 ease-in-out md:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
 
 {/* Mobile Close Button */}
 <button className="md:hidden absolute top-4 right-4 text-gray-500 hover:text-[var(--color-text-primary)]" onClick={onClose}>
 ...
 isActive 
 ? "bg-gradient-to-tr from-orange-500 to-red-400 text-white " 
