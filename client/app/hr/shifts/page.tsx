...
export default function ShiftPlannerPage() {
 ...
 <span className="text-sm font-semibold bg-white px-2 py-0.5 rounded-full ">{shifts.length} Staff</span>
 </div>
 ...
 shifts.map(shift => (
 <div key={shift.id} className="bg-white p-3 rounded-lg border border-gray-100 flex justify-between items-center group hover:border-blue-300 transition cursor-pointer">
 <div className="flex items-center gap-3">
 <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-xs font-semibold text-[var(--color-text-secondary)]">
 ...
 <p className="text-xs font-mono text-gray-700 bg-[var(--color-bg-tertiary)] px-2 py-1 rounded">
 ...
 <div>
 <h1 className="text-3xl font-semibold text-gray-900">Shift Planner</h1>
 <p className="text-gray-500 mt-1">Manage staff schedules, swaps, and coverage for <strong className="text-[var(--color-text-primary)]">{selectedDate}</strong>.</p>
 </div>
 ...
 <p className="text-xs font-semibold text-orange-800 uppercase tracking-wider flex gap-1 items-center"><span>🌅</span> Morning Shift</p>
 <p className="text-2xl font-semibold text-orange-900 mt-1">{metrics.morning || 0} <span className="text-sm font-normal text-orange-700">staff</span></p>
 </div>
 <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-4 rounded-xl border border-blue-200">
 <p className="text-xs font-semibold text-blue-800 uppercase tracking-wider flex gap-1 items-center"><span>☀️</span> Afternoon Shift</p>
 <p className="text-2xl font-semibold text-blue-900 mt-1">{metrics.evening || 0} <span className="text-sm font-normal text-blue-700">staff</span></p>
 </div>
 <div className="bg-gradient-to-br from-indigo-50 to-indigo-100 p-4 rounded-xl border border-indigo-200">
 <p className="text-xs font-semibold text-indigo-800 uppercase tracking-wider flex gap-1 items-center"><span>🌙</span> Night Shift</p>
 <p className="text-2xl font-semibold text-indigo-900 mt-1">{metrics.night || 0} <span className="text-sm font-normal text-indigo-700">staff</span></p>
 </div>
 <div className="bg-white p-4 rounded-xl border border-gray-200">
 <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Unassigned</p>
 <p className="text-2xl font-semibold text-gray-900 mt-1">{metrics.unassigned || 0}</p>
 </div>
 <div className="bg-white p-4 rounded-xl border border-gray-200">
 <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Swap Requests</p>
 <p className="text-2xl font-semibold text-red-600 mt-1">{metrics.swap_requests || 0}</p>
 </div>
 </div>
 ...
 <input 
 type="date" 
 className="p-2 border border-gray-200 rounded-lg text-sm bg-[var(--color-bg-primary)] shadow-inner font-medium text-blue-900" 
 value={selectedDate}
 ...
 <input type="text" placeholder="Search employee..." className="p-2 border border-gray-200 rounded-lg text-sm flex-1" />
 </div>
 ...
 <div className="mt-6">
 <ShiftBlock title="Custom Shifts" icon="⚙️" shifts={customShifts} colorClass="bg-[var(--color-bg-primary)] border-gray-200 text-gray-900" />
 </div>
 )}
 ...
 <h3 className="font-semibold text-[var(--color-text-primary)] flex items-center gap-2">⚡ Quick Assign</h3>
 </div>
 ...
 <span className="font-semibold text-[var(--color-text-primary)]">{req.employee_details?.first_name}</span>
 <span className="text-xs bg-purple-100 text-purple-800 px-2 rounded-full font-semibold">Pending</span>
 </div>
 <div className="flex items-center gap-2 text-xs text-[var(--color-text-secondary)] bg-[var(--color-bg-primary)] p-2 rounded">
 <span className="line-through">{req.current_shift_details?.name}</span>
 <span>➡️</span>
 <span className="font-semibold text-gray-900">{req.requested_shift_details?.name}</span>
 </div>
