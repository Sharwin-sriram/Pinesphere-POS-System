...
export default function EmployeeMobileApp() {
  ...
  if (loading) {
    return <div className="min-h-screen bg-gray-100 p-6 flex items-center justify-center">Loading your dashboard...</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-gray-100 font-sans text-gray-800 pb-20">
      ...
      {/* Header Profile */}
      <div className="bg-indigo-600 p-6 pt-12 text-white shadow-md rounded-b-3xl">
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 bg-white/20 rounded-full flex items-center justify-center text-2xl">
            👤
          </div>
          <div>
            <h1 className="text-2xl font-bold">{data?.name || "Employee"}</h1>
      ...
      <div className="max-w-md mx-auto px-4 mt-6 space-y-4">
        ...
        {!showCamera ? (
          ...
          <div className="bg-black p-4 rounded-2xl shadow-xl text-center">
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
          ...
          {data?.next_shift ? (
            <div>
              <p className="text-lg font-bold text-gray-900">{data.next_shift.name}</p>
              <p className="text-gray-600">{data.next_shift.date} • {data.next_shift.start_time} - {data.next_shift.end_time}</p>
            </div>
          ) : (
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
            ...
            <p className="text-2xl font-bold mt-1">{data?.casual_leaves || 0} Casual</p>
          </div>
          <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
            <h3 className="text-xs font-semibold text-gray-500 uppercase">Performance</h3>
            <p className="text-xl font-bold mt-1 text-indigo-600">{data?.performance_score || "N/A"}</p>
          </div>
        </div>

        {/* Actions List */}
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mt-4">
          <button className="w-full text-left p-4 border-b border-gray-50 flex justify-between items-center hover:bg-gray-50 transition">
          ...
          <button className="w-full text-left p-4 flex justify-between items-center hover:bg-gray-50 transition">
