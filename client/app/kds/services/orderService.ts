...
async function getKitchenContext() {
  const systemsResponse = await httpClient
    .get("/api/kds/systems/")
    .catch(async () => {
...
export async function getKdsBootstrap(): Promise<KdsBootstrap> {
  try {
    const { system, kitchen } = await getKitchenContext();
    ...
    const summaryResponse = await httpClient.get(
      ...
      httpClient.get("/api/kitchen/dashboard/summary/", {
        params: {
          kitchen_id: kitchen.id,
        },
      })
    ...
    return {
      ...
      stats: {
        ...
        preparing:
          (summary.total_preparing || 0) + (summary.total_pending || 0),
