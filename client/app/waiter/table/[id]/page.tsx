  ...
  return (
    <div className="flex flex-col h-full relative animate-fade-in-up pb-20">
      ...
      <div className="flex items-center gap-4 mb-6">
        <button onClick={() => router.back()} className="p-2 bg-white rounded-xl shadow-sm text-gray-500 hover:text-blue-600 transition-colors">
          <FiArrowLeft size={20} />
        </button>
        <div>
          <h2 className="text-2xl font-bold text-gray-800">Menu for Table {tableId}</h2>
          ...
          return (
            <div 
              ...
              className={`card-light !p-4 flex justify-between items-center transition-all ${
                isSelected ? 'border-blue-400 shadow-md bg-blue-50/30' : 'hover:border-gray-300'
              }`}
            >
              <div className="flex-1 pr-4">
                <div className="flex justify-between items-start mb-1">
                  <h3 className="font-bold text-gray-800">{dish.name}</h3>
                  <span className="font-bold text-blue-600">${dish.price.toFixed(2)}</span>
                </div>
              ...
              <button 
                ...
                className={`flex-shrink-0 w-24 py-2 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-1 ${
                  isSelected 
      ...
      {selectedItems.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 md:left-28 bg-white/80 backdrop-blur-md border-t border-gray-200 p-4 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] z-40 animate-fade-in-up">
          <div className="max-w-5xl mx-auto flex items-center justify-between">
            ...
            <button 
              ...
              className="bg-gradient-to-tr from-blue-500 to-cyan-400 text-white px-8 py-3 rounded-xl font-bold shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all flex items-center gap-2"
            >
              <FiShoppingCart />
              Submit Order
            </button>
