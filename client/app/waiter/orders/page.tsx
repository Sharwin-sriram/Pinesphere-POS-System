...
import { FiCheckCircle, FiClock, FiGrid, FiDollarSign, FiList, FiXCircle } from "react-icons/fi";
      ...
      {Object.keys(groupedOrders).length === 0 ? (
        ...
        <div className="flex flex-col gap-8">
          {Object.entries(groupedOrders).map(([tableNumber, tableOrders]) => {
            ...
            return (
              <div key={tableNumber} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                <div className="flex items-center justify-between mb-4 pb-4 border-b border-gray-100">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center font-bold text-lg shadow-inner">
                      <FiGrid size={18} className="mr-1" /> {tableNumber.replace('T', '')}
                    </div>
                    <div>
                      <h3 className="text-lg font-bold text-gray-800">Table {tableNumber}</h3>
                      <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${allServed ? 'bg-green-100 text-green-600' : 'bg-orange-100 text-orange-600'}`}>
                  ...
                  <button 
                    ...
                    className="flex items-center gap-2 bg-gradient-to-tr from-green-500 to-emerald-400 text-white px-4 py-2 rounded-lg font-bold shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all text-sm"
                  >
                    ...
                    return (
                      <div 
                        ...
                        className={`border rounded-xl p-4 flex flex-col transition-all ${
                          isServed ? 'border-gray-200 bg-gray-50 opacity-80' : 'border-blue-200 bg-blue-50/30'
                        }`}
                      >
                        <div className="flex justify-between items-start mb-3">
                          <div>
                            <span className="text-xs font-bold text-blue-600 bg-blue-100 px-2 py-1 rounded-md block mb-1 w-fit">
                              Order {index + 1}
                            </span>
                          ...
                          <div className="flex items-center gap-2">
                            {order.status === "Pending" && (
                              <span className="text-xs font-bold text-orange-600 flex items-center gap-1 bg-orange-100 px-2 py-1 rounded-md shadow-sm">
                                <FiClock /> Cooking
                              </span>
                            )}
                            ...
                            {order.status === "Cooked" && (
                              <button 
                                ...
                                className="text-xs font-bold bg-green-500 text-white px-3 py-1.5 rounded-md hover:bg-green-600 transition-colors shadow-sm animate-pulse"
                              >
                                Pick up & Serve
                              </button>
                            )}
                            ...
                            {order.status !== "Served" && (
                              <button 
                                ...
                                className="text-xs font-bold bg-red-100 text-red-600 px-3 py-1.5 rounded-md hover:bg-red-200 transition-colors flex items-center gap-1"
                                title="Cancel Order"
                              >
                                <FiXCircle /> Cancel
                              </button>
