        ...
        {tables.map((table) => {
          ...
          return (
            <Link
              ...
              key={table.id}
              className={`card-light !p-6 flex flex-col items-center justify-center cursor-pointer transition-all hover:-translate-y-1 hover:shadow-lg ${cardClasses}`}
            >
              <div className={`w-16 h-16 rounded-full flex items-center justify-center text-2xl font-bold mb-3 shadow-inner ${circleClasses}`}>
                {table.number}
              </div>
              ...
              {table.status !== 'Available' && (
                <div className="flex items-center gap-1 text-xs text-gray-500 mt-3 font-medium">
                  <FiUsers size={12} /> {table.pax} seated
                </div>
              )}
            </Link>
          );
